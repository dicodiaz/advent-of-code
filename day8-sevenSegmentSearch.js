...
const getCountOfUniqueDigits = (input) =>
  input.reduce(
    (totalSum, elem) =>
      totalSum +
      elem
        .split(' | ')[1]
        .split(' ')
        .reduce((sum, curr) => ([2, 3, 4, 7].includes(curr.length) ? sum + 1 : sum), 0),
    0,
  );

console.log(getCountOfUniqueDigits(input));

const getCountOfUniqueDigits2 = (input) =>
  input.reduce((sum, elem) => {
    const inputArr = elem.split(' | ');
    const codedInputDigits = inputArr[0]
      .split(' ')
      .map((elem) => elem.split('').sort().join(''))
      .sort((a, b) => a.length - b.length);
    const codedOutputDigits = inputArr[1].split(' ').map((elem) => elem.split('').sort().join(''));
    const keyMap = {};
    const keyMapReverse = {};
    keyMap[codedInputDigits[0]] = 1;
    keyMap[codedInputDigits[1]] = 7;
    keyMap[codedInputDigits[2]] = 4;
    keyMap[codedInputDigits[9]] = 8;
    keyMapReverse[1] = codedInputDigits[0];
    keyMapReverse[7] = codedInputDigits[1];
    keyMapReverse[4] = codedInputDigits[2];
    keyMapReverse[8] = codedInputDigits[9];
    getFiveSegmentDigitsIndexes(codedInputDigits, keyMap, keyMapReverse);
    getSixSegmentDigitsIndexes(codedInputDigits, keyMap, keyMapReverse);
    const decodedNumber = parseInt(codedOutputDigits.map((digit) => keyMap[digit]).join(''));
    return sum + decodedNumber;
  }, 0);

const getFiveSegmentDigitsIndexes = (codedInputDigits, keyMap, keyMapReverse) => {
  const fiveSegmentDigitsIndexes = [3, 4, 5];
  const regexOne = new RegExp(`[${keyMapReverse[1]}]`, 'g');
  const regexFour = new RegExp(`[${keyMapReverse[4]}]`, 'g');
  let i = 0;
  let count = 0;
  while (count < 3) {
    if (
      !keyMapReverse[3] &&
      codedInputDigits[fiveSegmentDigitsIndexes[i]].replace(regexOne, '').length === 3
    ) {
      keyMap[codedInputDigits[fiveSegmentDigitsIndexes[i]]] = 3;
      keyMapReverse[3] = codedInputDigits[fiveSegmentDigitsIndexes[i]];
      count++;
    } else if (
      !keyMapReverse[2] &&
      codedInputDigits[fiveSegmentDigitsIndexes[i]].replace(regexFour, '').length === 3
    ) {
      keyMap[codedInputDigits[fiveSegmentDigitsIndexes[i]]] = 2;
      keyMapReverse[2] = codedInputDigits[fiveSegmentDigitsIndexes[i]];
      count++;
    } else if (
      keyMapReverse[3] &&
      keyMapReverse[2] &&
      !Object.keys(keyMap).includes(codedInputDigits[fiveSegmentDigitsIndexes[i]])
    ) {
      keyMap[codedInputDigits[fiveSegmentDigitsIndexes[i]]] = 5;
      keyMapReverse[5] = codedInputDigits[fiveSegmentDigitsIndexes[i]];
      count++;
    }
    i = (i + 1) % 3;
  }
...
const getSixSegmentDigitsIndexes = (codedInputDigits, keyMap, keyMapReverse) => {
  const sixSegmentDigitsIndexes = [6, 7, 8];
  const regexOne = new RegExp(`[${keyMapReverse[1]}]`, 'g');
  const regexThree = new RegExp(`[${keyMapReverse[3]}]`, 'g');
  let i = 0;
  let count = 0;
  while (count < 3) {
    if (
      !keyMapReverse[6] &&
      codedInputDigits[sixSegmentDigitsIndexes[i]].replace(regexOne, '').length === 5
    ) {
      keyMap[codedInputDigits[sixSegmentDigitsIndexes[i]]] = 6;
      keyMapReverse[6] = codedInputDigits[sixSegmentDigitsIndexes[i]];
      count++;
    } else if (
      !keyMapReverse[9] &&
      codedInputDigits[sixSegmentDigitsIndexes[i]].replace(regexThree, '').length === 1
    ) {
      keyMap[codedInputDigits[sixSegmentDigitsIndexes[i]]] = 9;
      keyMapReverse[9] = codedInputDigits[sixSegmentDigitsIndexes[i]];
      count++;
    } else if (
      keyMapReverse[6] &&
      keyMapReverse[9] &&
      !Object.keys(keyMap).includes(codedInputDigits[sixSegmentDigitsIndexes[i]])
    ) {
      keyMap[codedInputDigits[sixSegmentDigitsIndexes[i]]] = 0;
      keyMapReverse[0] = codedInputDigits[sixSegmentDigitsIndexes[i]];
      count++;
    }
    i = (i + 1) % 3;
  }
...
console.log(getCountOfUniqueDigits2(input));
